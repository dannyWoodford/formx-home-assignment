...
export default function Cube() {
  ...
  const brickTextures = useTexture(MATERIAL_DEFINITIONS.brick.textures ?? {}) as Partial<
    Record<keyof MaterialTextures, Texture>
  >
  const metalTextures = useTexture(MATERIAL_DEFINITIONS.metal.textures ?? {}) as Partial<
    Record<keyof MaterialTextures, Texture>
  >
  ...
  const handlePresetChange = useCallback(
    (faceIndex: number, nextPreset: FaceMaterialPreset) => {
      setFacePresets((prev) => {
  ...
  return (
    <mesh position={[0, 0, 0]} castShadow onPointerDown={handlePointerDown}>
      ...
      {material === 'Basic' && <meshBasicMaterial color={'mediumpurple'} />}
      {material === 'PBR' && (
        <>
          {faceMaterials}

          {/* Using drei's <Html /> keeps the panel local to this component without wiring a global UI layer. Transform is disabled so it behaves like HUD UI even though it's still part of the scene graph. */}
          {/* Only did it here for demo speed; a production build would route this through the real UI and sync with state management of choice. */}
          <Html
            wrapperClass='face-select-container-wrapper'
            className='face-select-container'
            fullscreen
            transform={false}
          >
